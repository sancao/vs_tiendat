                            ...
                            <div class="form-group col-md-12">
                                ...
                                <div class="select is-fullwidth">
                                    <select class="form-control" v-model="chonso.daiduthuong" id="inputDaiDuThuong"
                                        ...
                                        <option v-for="d in getDanhSachDai" :value="d.value" 
...
var axios = require("axios");
  export default {
    ...
    created(){
        this.danh_sach_dai()
        this.getChonsosPaging(this.pagination.current_page);
    },
    mounted() {
        let vm=this;
        debugger
    ...
    computed: {
        getDanhSachDai(){
            debugger
            let data= this.$store.state.chonsos.ds_dai;
            return data.items
        },
    ...
    methods: {
        async inve(id){
            app.$store.dispatch('chonsos/invePdf',id).then((response) => {
        ...
        danh_sach_dai(){
            this.$store.dispatch('chonsos/getDanhSachDai');
        },
        saveChonso() {
            var app = this;
            app.$v.chonso.$touch();
            if (!app.$v.chonso.$invalid) 
            {
                app.$store.dispatch('chonsos/addOrUpdate',app.chonso)
                .then(function (resp) {
                    debugger
                    app.getChonsosPaging(app.pagination.current_page);
        ...
        async getChonsosPaging(page){
            let vm = this;
            vm.$store.dispatch('chonsos/getChonsoAllAndPaging',page,vm.searchValue)
            .then(function (resp) {
        ...
        getInitPage(data){
            debugger
            let vm=this
            vm.pagination.from= data.items.from;
            vm.pagination.to= data.items.to;
            vm.pagination.per_page= data.items.per_page;
            vm.pagination.total= data.items.total;
            vm.pagesNumber=vm.getPagesNumber(data.items);
            vm.isActived=vm.getIsActived(data.items);
        },
